...
namespace bae
{
	...
	public:
		...
		size_t FindRootNodesThat(in<TNodePredicate> predicate, ref_nullable<TResultCollection> results)
		{
			static_assert(std::is_base_of<Node, T>::value);
			size_t foundCount = 0;
		...
		template<typename T, typename TNodePredicate, typename TResultCollection>
		size_t FindNodesThat(in<TNodePredicate> predicate, ref_nullable<TResultCollection> results)
		{
			static_assert(std::is_base_of<Node, T>::value);
			size_t foundCount = 0;
		...
		size_t FindCRootNodesOfType(ref_nullable<TResultCollection> results)
		{ return FindRootNodesThat<T, auto, TResultCollection>([](T*) -> bool { return true; }, results); }

		template<typename T, typename TResultCollection>
		size_t FindNodesOfType(ref_nullable<TResultCollection> results)
		...
		template<typename T, typename TResultCollection>
		size_t FindRootNodesWithName(in<std::string> name, ref_nullable<TResultCollection> results)
		{ return FindRootNodesThat<T>([&](T* node) -> bool { return node->GetName() == name; }); }

		template<typename T, typename TResultCollection>
		size_t FindNodesWithName(in<std::string> name, ref_nullable<TResultCollection> results)
